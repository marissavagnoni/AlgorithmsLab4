...
def word_count_map(doc):
    ...
    """
    words = doc.split() #split document into words using spaces 

  #creates list of (word, 1) pairs 
    result = [] 
    for word in words:
...
def word_count_reduce(group):
    ...
    total = reduce(lambda x, y: x + y, 0, counts)
...
def sentiment_map(doc,
    ...
    """
    result = [] #creates empty list to store tuples
    for word in doc.split(): #splits input string looping through each word
        if word in pos_terms:
            result.append(('positive', 1)) #checks if word is in set of positive terms and appends it if so
        elif word in neg_terms:
            result.append(('negative', 1)) #checks if word is in set of negative terms and appends it if so
    return result
