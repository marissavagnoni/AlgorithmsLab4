...
**Work: O(n) since we sum n ones and do n - 1 additions. Span: O(logn) since the list is cut in half each time.**
...
**The function above uses reading, splitting, and counting in one place at the same time, which makes it hard for machines and processors to accomplish splitting the work. Also, the counts dictionary is constantly updated so if multiple sources are trying to update it simultaneously an error can occur and slow down the process. Lastly map-reduce combines the results more efficiently and safely making it easier to run big data systems.**
